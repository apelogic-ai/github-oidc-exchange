...
| Item | Contract |
| --- | --- |
| Issuer | Exact HTTPS origin in `config.issuerUrl`, without a path, query, fragment, or trailing slash. Runtime and chart validation reject path-bearing issuer URLs. Consumers pin the issuer exactly. |
