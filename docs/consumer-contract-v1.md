...
| Item | Contract |
| --- | --- |
| Issuer | Exact HTTPS `config.issuerUrl`, without a trailing slash. Consumers pin it exactly. In the discovery formula below, `{origin}` is its scheme plus authority and `{issuer-path}` is its path component. |
| Discovery | RFC 8414: `GET {origin}/.well-known/oauth-authorization-server{issuer-path}`, where `{issuer-path}` is empty for a root issuer. Thus `https://identity.example.org/tenant` is discovered at `https://identity.example.org/.well-known/oauth-authorization-server/tenant`. The retained `GET {origin}/.well-known/openid-configuration` endpoint returns the same document. `issuer`, `jwks_uri`, `token_endpoint`, `github_oidc_exchange_endpoint`, `github_oidc_audience`, `identity_contracts_supported`, and `policy_versions_supported` are authoritative. |
